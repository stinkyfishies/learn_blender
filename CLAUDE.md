...
my_blender_project/
├── CLAUDE.md        # this file
├── .gitignore       # *.blend, *.blend1, renders/, __pycache__/
├── README.md        # what each script produces and how to run it
├── scenes/          # one .py per scene or deliverable — self-contained, runnable
├── lib/             # reusable functions imported by scene scripts
│   ├── lighting.py  # lighting rig functions
│   ├── materials.py # material setup functions
│   └── utils.py     # shared helpers (clear_scene, etc.)
└── renders/         # output — gitignored
```

- `.blend` files are build artifacts (generated by running scripts). Gitignore them.
- Scene scripts in `scenes/` import from `lib/` rather than duplicating setup code.
- New scene scripts follow the structure of existing ones in `scenes/`.

## Conventions

- Objects: `snake_case`. Materials: `Title_Case`.
- Target render output: 1920x1080, 24fps.
- Render output path: `renders/` (relative to project root).
- Each new function added to `lib/` should have a one-line docstring describing what it produces.
